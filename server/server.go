...
import (
	...
	"github.com/AliyunContainerService/kube2ram/iptables"
	"github.com/AliyunContainerService/kube2ram/k8s"
	"github.com/AliyunContainerService/kube2ram/mappings"
...
const (
	...
	defaultRAMRoleKey                 = "ram.aliyuncs.com/role"
	defaultLogLevel                   = "info"
	defaultLogFormat                  = "text"
	defaultMaxElapsedTime             = 2 * time.Second
	defaultRAMRoleSessionTTL          = 15 * time.Minute
...
func (s *Server) healthHandler(logger *log.Entry, w http.ResponseWriter, r *http.Request) {
	...
	// without fear of overwhelming any rate limits with Alibaba Cloud or other dependencies.
...
func (s *Server) securityCredentialsHandler(logger *log.Entry, w http.ResponseWriter, r *http.Request) {
	...
	if s.ram.BaseARN != "" && strings.HasPrefix(roleMapping.Role, s.ram.BaseARN) {
		write(logger, w, strings.TrimPrefix(roleMapping.Role, s.ram.BaseARN))
		return
	}
...
func (s *Server) roleHandler(logger *log.Entry, w http.ResponseWriter, r *http.Request) {
	...
	roleLogger := logger.WithFields(log.Fields{
		"pod.ram.role": roleMapping.Role,
		"ns.name":      roleMapping.Namespace,
	})

	wantedRole := mux.Vars(r)["role"]
	wantedRoleARN := s.ram.RoleARN(wantedRole)

	if wantedRoleARN != roleMapping.Role && !strings.HasSuffix(s.BaseRoleARN, strings.ToLower(wantedRole)) {
		roleLogger.WithField("params.ram.role", wantedRole).
			Error("Invalid role: does not match annotated role")
		http.Error(w, fmt.Sprintf("Invalid role %s", wantedRole), http.StatusForbidden)
		return
	}

	credentials, err := s.ram.AssumeRole(wantedRoleARN, remoteIP, s.RAMRoleSessionTTL)
	if err != nil {
		roleLogger.Errorf("Error assuming role %+v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	roleLogger.Debugf("retrieved credentials from sts endpoint: %s", s.ram.Endpoint)
...
func (s *Server) reverseProxyHandler(logger *log.Entry, w http.ResponseWriter, r *http.Request) {
	...
	proxy.ServeHTTP(w, r)
	logger.WithField("metadata.url", s.MetadataAddress).Debug("Proxy ECS metadata request")
}
...
func (s *Server) Run(host, token, nodeName string, insecure bool, accessKey, accessSecret string) error {
	...
	s.k8s = k
	config, err := ram.GetCloudConfigFromMetaServer()
	if err != nil {
		return err
	}
	log.Printf("getCloudConfigFromStsToken: %++v", config)
	...
	s.ram = ram.NewClient(accessKey, accessSecret, s.BaseRoleARN, config)
	log.Debugln("Caches have been synced.  Proceeding with server.")
	s.roleMapper = mappings.NewRoleMapper(s.RAMRoleKey, s.DefaultRAMRole, s.NamespaceRestriction, s.NamespaceKey, s.ram, s.k8s, s.NamespaceRestrictionFormat)
	podSynched := s.k8s.WatchForPods(kube2ram.NewPodHandler(s.RAMRoleKey))
	namespaceSynched := s.k8s.WatchForNamespaces(kube2ram.NewNamespaceHandler(s.NamespaceKey))
	...
	if err := http.ListenAndServe(":"+s.AppPort, r); err != nil {
		log.Fatalf("Error creating kube2ram http server: %+v", err)
	}
	return nil
}
